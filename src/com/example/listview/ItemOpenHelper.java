...
public class ItemOpenHelper extends SQLiteOpenHelper {
	...
	public ItemOpenHelper(Context context, String name,
	...
	public void setNewItemInsert() throws MalformedURLException, XmlPullParserException, IOException{
		...
		int dbVersion=0;
		List<Map<String,String>> versionMap = XmlUtil.getXml(new URL("http://doryu.dix.asia/get_item_max_version.php"));
		if(versionMap.size()==1){
			dbVersion =  Integer.parseInt(versionMap.get(0).get("item_version"));
		}
		if(dbVersion>userVersion){
			List<Map<String,String>> updateMap = XmlUtil.getXml(new URL("http://doryu.dix.asia/get_item_v_to_v.php?user_v="+userVersion+"&db_v="+dbVersion));
